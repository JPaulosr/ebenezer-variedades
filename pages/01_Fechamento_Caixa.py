...
# ========= COGS (custo médio ponderado do histórico) =========
def _custo_medio_map(comp_df: pd.DataFrame, prod_df: pd.DataFrame) -> dict:
    if comp_df.empty and (prod_df.empty or "CustoAtual" not in prod_df.columns):
        return {}
    mp = {}
    if not comp_df.empty:
        ...
        if col_idp and col_qtd and col_cu:
            ...
            g = c.groupby(c[col_idp].astype(str))[["Parcial","QtdNum"]].sum()
            g["CustoMedio"] = g["Parcial"] / g["QtdNum"].replace(0, pd.NA)
            mp = g["CustoMedio"].fillna(0.0).to_dict()
    # fallback: Produtos.CustoAtual
    if not prod_df.empty and "ID" in prod_df.columns and "CustoAtual" in prod_df.columns:
        for _id, custo in prod_df.set_index("ID")["CustoAtual"].items():
            if _id not in mp or mp[_id] == 0:
                mp[str(_id)] = float(custo) if pd.notna(custo) else 0.0
    return mp
...
if vendas.empty:
    ...
    receita = cupom["ReceitaCupom"].sum()            # LÍQUIDO (TotalCupom OU Total - Desconto)
    bruto   = cupom["TotalNum"].sum()                 # bruto por cupom
    desc_tot= max(0.0, bruto - receita)

# COGS do período: soma(qtd vendida * custo médio)
if vendas.empty or not custo_map:
    cogs = 0.0
else:
    cogs = ventas_cogs = (vendas["QtdNum"] * vendas["IDProduto"].map(lambda x: float(custo_map.get(str(x),0.0)))).sum()
...
if not cupom.empty:
    ...
    st.dataframe(fpg.rename(columns={"Forma":"Forma de pagamento","ReceitaCupom":"Total (R$)"}), use_container_width=True, hide_index=True)
else:
    st.info("Sem vendas no período.")

st.divider()

# ========= Resumo por produto (alocando desconto proporcional por linha) =========
...
if vendas.empty:
    ...
    if not key:
        st.info("Não encontrei coluna de ID do produto nas vendas.")
    else:
        # junta ao cupom para obter ReceitaCupom e Bruto do cupom
        cup_map = cupom[["VendaID","ReceitaCupom","TotalNum"]].rename(columns={"TotalNum":"BrutoCupom"})
        tmp = vendas.merge(cup_map, how="left", on="VendaID")
        # receita líquida por linha = bruto da linha * (receita do cupom / bruto do cupom)
        ...
        # agrega por produto
        base = tmp[[key, "QtdNum", "ReceitaLinha", "TotalNum"]].copy()  # mantém TotalNum se quiser comparar
        base[key] = base[key].astype(str)
        grp = (
            base.groupby(key, dropna=False)
                .agg(Qtd=("QtdNum","sum"),
        ...
        # COGS com custo médio e lucro
        grp["COGS"]  = grp.apply(lambda r: r["Qtd"] * float(custo_map.get(str(r[key]), 0.0)), axis=1)
