...
if produtos.empty or COL_UNID not in produtos.columns:
    ...
    if df_granel.empty:
        ...
        with c1:
            ...
            vol_1_l = st.number_input("Volume por frasco A (em L) — ex.: 1.0", min_value=0.0, step=0.1, value=1.0, format="%.3f", key=f"vol_a_{BUMP}")
        with c2:
            ...
            vol_2_l = st.number_input("Volume por frasco B (em L) — ex.: 0.5", min_value=0.0, step=0.1, value=0.5, format="%.3f", key=f"vol_b_{BUMP}")
